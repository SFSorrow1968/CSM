...
if (-not (Test-Path "bin/PCVR/CSM/CSM.dll")) { Write-Error "PCVR Build Failed: DLL not found." }
if (-not (Test-Path "bin/Nomad/CSM/CSM.dll")) { Write-Error "Nomad Build Failed: DLL not found." }

# 4. Package
Write-Host "4. Packaging..." -ForegroundColor Cyan
$pcvrZip = "CSM_PCVR_v$version.zip"
$nomadZip = "CSM_Nomad_v$version.zip"

if (Test-Path $pcvrZip) { Remove-Item $pcvrZip }
if (Test-Path $nomadZip) { Remove-Item $nomadZip }

Compress-Archive -Path "bin/PCVR/CSM/*" -DestinationPath $pcvrZip
Compress-Archive -Path "bin/Nomad/CSM/*" -DestinationPath $nomadZip
